...
json_t* Quant::dataToJson()
{
    ...
    json_object_set_new(root, "relative", json_boolean(this->relative));
    return root;
}

void Quant::dataFromJson(json_t* root)
{
    json_t * object = json_object_get(root, "relative");
    this->relative = object ? json_boolean_value(object) : false;
}
...
float Quant::getOctave()
{
    ...
    if (! this->inputs[INPUT_OCTAVE].isConnected())
        ...
        if (relative) voltage += this->params[PARAM_OCTAVE].getValue();
...
float Quant::getRoot(Scale * scale)
{
    ...
    if (! this->inputs[INPUT_ROOT].isConnected())
        ...
        root = clamp(this->inputs[INPUT_ROOT].getVoltage(), 0.0f, 1.0f);
...
struct QuantWidget : ModuleWidget
{
    ...
    void appendContextMenu(Menu* menu) override {
        ...
        menu->addChild(createBoolPtrMenuItem("Relative octave", "", &m->relative));
    }
};
