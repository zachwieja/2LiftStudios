//  Copyright (c) 2026, 2 Lift Studios
//  All rights reserved.
...
struct Sand2 : ThemeModule {
    ...
    public:
        Sand2() : uniform(-1.0, 1.0)
        {
            ...
            configSwitch(PARAM_MODE1, Mode::MODE_FIRST, Mode::MODE_LAST, Mode::MODE_DEFAULT, "Mode", {"Track", "Track Low", "Track High", "Sample & Hold" });
            configInput(INPUT_GATE1, "Gate");
            configButton(PARAM_GATE1, "Manual");
            configInput(INPUT_POLY1, "In");
            configOutput(OUTPUT_POLY1, "Out");

            configSwitch(PARAM_MODE2, Mode::MODE_FIRST, Mode::MODE_LAST, Mode::MODE_DEFAULT, "Mode", {"Track", "Track Low", "Track High", "Sample & Hold" });
            configInput(INPUT_GATE2, "Gate");
            configButton(PARAM_GATE2, "Manual");
            configInput(INPUT_POLY2, "In");
            configOutput(OUTPUT_POLY2, "Out");
        }
...
struct Sand2Widget : ThemeWidget<Sand2>
{
    Sand2Widget(Sand2 * module) : ThemeWidget<Sand2>(module, "Sand2")
    {
        setModule(module);
        ...
        this->addParam(createParamCentered<Trimpot>(mm2px(Vec(x, y)), module, Sand2::PARAM_MODE1));
        this->addInput(createInputCentered<PJ301MPort>(mm2px(Vec(x, y += dy)), module, Sand2::INPUT_GATE1));
        this->addParam(createParamCentered<TinyTrigger>(mm2px(Vec(x + 4.0f, y + 6.25f)), module, Sand2::PARAM_GATE1));
        this->addInput(createInputCentered<PJ301MPort>(mm2px(Vec(x, y += dy)), module, Sand2::INPUT_POLY1));
        this->addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(x, y + dy + 0.5f)), module, Sand2::OUTPUT_POLY1));

        y = 68.5;
        this->addParam(createParamCentered<Trimpot>(mm2px(Vec(x, y)), module, Sand2::PARAM_MODE2));
        this->addInput(createInputCentered<PJ301MPort>(mm2px(Vec(x, y += dy)), module, Sand2::INPUT_GATE2));
        this->addParam(createParamCentered<TinyTrigger>(mm2px(Vec(x + 4.0f, y + 6.25f)), module, Sand2::PARAM_GATE2));
        this->addInput(createInputCentered<PJ301MPort>(mm2px(Vec(x, y += dy)), module, Sand2::INPUT_POLY2));
        this->addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(x, y + dy + 0.5f)), module, Sand2::OUTPUT_POLY2));
    }
    ...
    {
        Sand2 * module = dynamic_cast<Sand2 *>(this->module);
        ...
        menu->addChild(createIndexSubmenuItem(
            ...
            [=](int noise) { module->noise = (Sand2::Noise) noise; }
        ));

        labels.clear();
        labels.push_back("[-1V, 1V]");   // 0
        labels.push_back("[-3V, 3V]");   // 1
        labels.push_back("[-5V, 5V]");   // 2
        labels.push_back("[-10V, 10V]"); // 3
...
Model * modelSand2 = createModel<Sand2, Sand2Widget>("Sand2");
